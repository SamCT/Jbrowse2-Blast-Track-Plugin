...
export default function BlastSelectionDialog({ handleClose, mode, model, regions, }) {
    ...
    async function runBlastpGenes(source) {
        ...
        if (!genes.length) {
            throw new Error(`No gene, mRNA, or transcript features found in visible feature tracks for ${regionLabel(region)}. Show a compatible gene annotation track for this assembly, then run BLASTP genes in selection again.`);
        }
